-- Active: schema.sql for Oráculo-LATAM (v2 - Admin & Cost Audit)
...
-- PROFILES TABLE
-- Stores user information, reputation, virtual ORC balance, and system roles.
CREATE TABLE IF NOT EXISTS public.profiles (
    id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    username TEXT NOT NULL UNIQUE,
    email TEXT,
    avatar_url TEXT,
    country TEXT NOT NULL, -- CO, MX, AR, BR, CL, PE, etc.
...
-- MARKETS TABLE
-- Stores prediction questions, prices (represented in ORCs 1-99), volume, and resolution details.
CREATE TABLE IF NOT EXISTS public.markets (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    title TEXT NOT NULL,
    description TEXT,
    category TEXT NOT NULL CHECK (category IN ('Política', 'Economía', 'Tecnología', 'Deportes', 'Cultura')),
    country TEXT NOT NULL, -- CO, MX, AR, BR, CL, PE, LATAM
...
-- TRANSACTIONS TABLE
-- Stores historical buying/selling activities of users.
CREATE TABLE IF NOT EXISTS public.transactions (
    ...
    points_paid NUMERIC(12, 2) NOT NULL, -- ORCs paid (buy) or received (sell)
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- USER POSITIONS TABLE
-- Stores the current shares a user holds for a particular market.
...
-- REWARDS TABLE
-- Stores elements that can be redeemed using ORCs.
...
-- REDEMPTIONS TABLE
-- Stores history of rewards redeemed by users.
...
-- AI COSTS TABLE
-- Tracks token consumption and costs for backend multi-agent auditing.
...
-- Stores historical prices of markets to draw probability graphs in real time.
...
-- Profiles: public read, admin write/update, user update restricted to non-critical fields
...
-- Markets: public read, write/delete only for admins, price update for active markets
...
-- Rewards: public read, update stock for redemption and admin full update
...
-- AI Costs: public read, write for admin
...
-- SERVER-SIDE TRIGGERS FOR BALANCES AND LEDGER INTEGRITY

-- Trigger for Transactions: Automatically update user balance on trade insert
...
-- Trigger for Redemptions: Automatically deduct points when user redeems a reward
...
(
    ...
    '¿Aprobará el Congreso de Colombia la reforma de salud antes de fin de año?',
    'Se resolverá a SÍ si el Congreso de la República de Colombia aprueba en último debate el proyecto de ley de la reforma a la salud antes del 31 de diciembre. Se resolverá a NO en caso contrario.',
    'Política',
    'CO',
    ...
    'https://images.unsplash.com/photo-1541872703-74c5e44368f9?auto=format&fit=crop&w=400&q=80'
),
(
    'c2c1f4e1-27d1-447b-a320-c7be0ad0e002',
    '¿Crecimiento del PIB de México superará el 2.5% anual en el reporte de Q3?',
    'Se resolverá a SÍ si el INEGI publica que el crecimiento acumulado del PIB real supera el 2.5% anual para el tercer trimestre de este año. Se resolverá a NO si el crecimiento es de 2.5% o menos.',
    'Economía',
    'MX',
...
(
    ...
    '¿Alcanzará la inflación anual en Argentina menos del 80% en diciembre?',
    'Se resolverá a SÍ si el INDEC reporta una tasa de inflación interanual acumulada menor al 80.0% en su informe de diciembre. Se resolverá a NO si es igual o superior.',
    'Economía',
    'AR',
    ...
    'https://images.unsplash.com/photo-1569025690938-a00729c9e1f9?auto=format&fit=crop&w=400&q=80'
),
(
    'c2c1f4e1-27d1-447b-a320-c7be0ad0e004',
    '¿Llegará una startup chilena a convertirse en Unicornio este semestre?',
    'Se resolverá a SÍ si alguna startup con sede principal en Chile anuncia una ronda de inversión que eleve su valoración a 1,000 millones de USD o más antes de que termine el semestre en curso.',
    'Tecnología',
    'CL',
...
(
    ...
    '¿Ganará Brasil la Copa América en la próxima edición?',
    'Se resolverá a SÍ si la selección masculina absoluta de fútbol de Brasil se consagra campeona oficial de la Copa América. Se resolverá a NO en caso contrario.',
    'Deportes',
    'BR',
...
(
    ...
    'Suscripción Digital a La República (Colombia)',
    '1 mes de acceso premium ilimitado a noticias de negocios, economía y finanzas del diario líder de Colombia.',
    5000.00,
...
(
    ...
    'Suscripción Digital a El Economista (México)',
    '1 mes de acceso digital premium a análisis macroeconómicos y mercados financieros de México y el mundo.',
    5000.00,
...
(
    ...
    'Suscripción a Valor Econômico (Brasil)',
    '1 mes de acceso ilimitado a noticias del mercado financiero y negocios en portugués.',
    6000.00,
...
(
    ...
    'Cupón Platzi Premium (Educación)',
    '1 mes de suscripción Platzi Plan Classic con acceso a todos los cursos de programación, marketing y finanzas.',
    10000.00,
...
-- Insert AI Cost Seed Data (Simulated usage history for cost dashboard)
...
-- Sets 'admin' role if the email is germanmoralesconsulting@gmail.com.
...
BEGIN
  ...
  -- Resolve username duplicates using a loop to ensure uniqueness
  ...
  VALUES (
    ...
    1000.00, -- Welcome gift of 1,000 ORCs
    0,
    0.00,
    0,
    CASE 
...
-- RPC FUNCTION FOR USERS TO DELETE THEIR OWN ACCOUNT
...
-- TRIGGER FOR PRICE HISTORY LOGGING
-- Automatically logs price history when a market's yes_price is updated.
...
CREATE OR REPLACE TRIGGER on_market_price_updated
  AFTER UPDATE ON public.markets
  FOR EACH ROW EXECUTE FUNCTION public.log_market_price_history();

-- ALTER PROFILE COLUMNS FOR AGE AND GENDER DEMOGRAPHICS
...
CREATE OR REPLACE TRIGGER on_resolved_payout_inserted
...
CREATE TABLE IF NOT EXISTS public.limit_orders (
    ...
    market_id UUID NOT NULL REFERENCES public.markets(id) ON DELETE CASCADE,
    outcome TEXT NOT NULL CHECK (outcome IN ('YES', 'NO')),
    limit_price NUMERIC(5,2) NOT NULL CHECK (limit_price >= 1.00 AND limit_price <= 99.00), -- en centavos, ej. 54.00
...
-- TRIGGER FOR LIMIT ORDERS BALANCE MANAGEMENT
...
BEGIN
  ...
  -- If placing a new pending order, lock the balance
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    -- If cancelled, refund the locked amount
    IF OLD.status = 'pending' AND NEW.status = 'cancelled' THEN
      UPDATE public.profiles
      SET orc_balance = orc_balance + order_cost
      WHERE id = NEW.profile_id;
    -- If filled, we refund the lock, and the transaction trigger will deduct the actual price
...
CREATE OR REPLACE TRIGGER on_limit_order_changed
